...
async function login(user) {
    ...
    let object = { user_id: userData.user_id, cart_id: userData.cart_id };
    ...
    let successfullLoginResponse = {
        ...
        cart_date: userData.cart_date,
        street: userData.street,
        total_price: userData.total_price,
        is_checked_out: userData.is_checked_out,
        order_date: userData.last_order,
    };
    if (userData.user_type == "ADMIN") {
        object.user_type = "ADMIN";
    } else {
        successfullLoginResponse.user_type = "CUSTOMER";
    }
...
async function addUser(user) {
    ...
    if (await usersDao.isUserIdExist(user.user_id)) {
